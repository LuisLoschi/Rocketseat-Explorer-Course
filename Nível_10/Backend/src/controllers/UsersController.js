...
const sqliteConnection = require("../database/sqlite"); //Importa banco de dados
...
class UsersController {
    async create(request, response) {
        ...
        //Verificar se usuário da existe
    ...
    async update(request, response) {
        ...
        const user_id = request.user.id; //ensureAuthenticated

        const database = await sqliteConnection();
        const user = await database.get("SELECT * FROM users WHERE id = (?)", [user_id]); //Selecionar o usuário
